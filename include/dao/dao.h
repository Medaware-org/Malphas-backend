...
template<typename T>
[[nodiscard]] inline typename std::enable_if<std::is_arithmetic<T>::value, std::string>::type xto_string(T arg)
{
        return std::to_string(arg);
}

template<typename T>
[[nodiscard]] inline typename std::enable_if<std::is_same<T, std::string>::value, std::string>::type xto_string(T arg)
{
        return arg;
}
                                      ...
                                      std::string passwd_hash)
{
        std::string query = "UPDATE \"user\" SET nickname = " + std::string("'") + xto_string(nickname) +
                            ...
                            std::string("'") + " WHERE id = '" + xto_string(id) + "';";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}
...
{
        std::string query = "INSERT INTO \"session\" (session_token, user_id) VALUES (" + std::string("'") +
                            xto_string(session_token) + std::string("'") + ", " + std::string("'") + xto_string(user_id)
                            + std::string("'") + ")";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}
...
[[nodiscard]] inline bool session_update(Database &db, std::string /*PK*/ session_token, std::string user_id)
{
        std::string query = "UPDATE \"session\" SET user_id = " + std::string("'") + xto_string(user_id) +
                            std::string("'") + " WHERE session_token = '" + xto_string(session_token) + "';";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}
                                         ...
                                         std::string gate_type)
{
        std::string query = "UPDATE \"circuit\" SET parent_scene = " + std::string("'") + xto_string(parent_scene) +
                            ...
                            xto_string(location_y) + ", parent_circuit = " + (parent_circuit
                                                                                      ? (std::string("'") +
                                                                                              xto_string(
                                                                                                      *parent_circuit) +
                                                                                              std::string("'"))
                                                                                      : "null") + ", gate_type = " +
                            std::string("'") + xto_string(gate_type) + std::string("'") + " WHERE id = '" + xto_string(
                                    id) + "';";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}
...
{
        std::string query =
                "SELECT * FROM \"user\" u INNER JOIN session s on s.user_id = u.id WHERE s.session_token = '" + user_id
                + "';";
        PGresult *res = dao_query(db, query, PGRES_TUPLES_OK);
        if (!res) return false;
        if (PQntuples(res) != 1) {
