...
[[nodiscard]] inline bool user_insert(Database &db, std::string /*PK*/ id, std::string nickname,
                                      std::string passwd_hash)
{
        std::string query = "INSERT INTO \"user\" (id, nickname, passwd_hash) VALUES (" + std::string("'") +
                            xto_string(id) + std::string("'") + ", " + std::string("'") + xto_string(nickname) +
                            std::string("'") + ", " + std::string("'") + xto_string(passwd_hash) + std::string("'") +
                            ")";
...
[[nodiscard]] inline bool user_update(Database &db, std::string /*PK*/ id, std::string nickname,
                                      std::string passwd_hash)
{
        std::string query = "UPDATE \"user\" SET nickname = " + std::string("'") + xto_string(nickname) +
                            std::string("'") + ", passwd_hash = " + std::string("'") + xto_string(passwd_hash) +
                            std::string("'") + " WHERE id = " + std::string("'") + xto_string(id) + std::string("'") +
                            ";";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}
...
[[nodiscard]] inline bool session_insert(Database &db, std::string /*PK*/ session_token, std::string user_id,
                                         std::optional<bool> invalidated)
{
        std::string query = "INSERT INTO \"session\" (session_token, user_id, invalidated) VALUES (" + std::string("'")
                            + xto_string(session_token) + std::string("'") + ", " + std::string("'") +
                            xto_string(user_id) + std::string("'") + ", " + (invalidated
                                                                                     ? (xto_string(*invalidated))
                                                                                     : "null") + ")";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}
...
[[nodiscard]] inline bool session_update(Database &db, std::string /*PK*/ session_token, std::string user_id,
                                         std::optional<bool> invalidated)
{
        std::string query = "UPDATE \"session\" SET user_id = " + std::string("'") + xto_string(user_id) +
                            std::string("'") + ", invalidated = " + (invalidated ? (xto_string(*invalidated)) : "null")
                            + " WHERE session_token = " + std::string("'") + xto_string(session_token) + std::string(
                                    "'") + ";";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}
...
[[nodiscard]] inline bool scene_insert(Database &db, std::string /*PK*/ id, std::string author, std::string scene_name,
                                       std::string description)
{
        std::string query = "INSERT INTO \"scene\" (id, author, scene_name, description) VALUES (" + std::string("'") +
                            xto_string(id) + std::string("'") + ", " + std::string("'") + xto_string(author) +
                            std::string("'") + ", " + std::string("'") + xto_string(scene_name) + std::string("'") +
                            ", " + std::string("'") + xto_string(description) + std::string("'") + ")";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}

[[nodiscard]] inline bool scene_update(Database &db, std::string /*PK*/ id, std::string author, std::string scene_name,
                                       std::string description)
{
        std::string query = "UPDATE \"scene\" SET author = " + std::string("'") + xto_string(author) + std::string("'")
                            + ", scene_name = " + std::string("'") + xto_string(scene_name) + std::string("'") +
                            ", description = " + std::string("'") + xto_string(description) + std::string("'") +
                            " WHERE id = " + std::string("'") + xto_string(id) + std::string("'") + ";";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}
...
[[nodiscard]] inline bool scene_save(Database &db, std::string /*PK*/ id, std::string author, std::string scene_name,
                                     std::string description)
{
        scene tmp;
...
[[nodiscard]] inline bool circuit_insert(Database &db, std::string /*PK*/ id, std::string parent_scene, int location_x,
                                         int location_y, std::optional<std::string> parent_circuit,
                                         std::string gate_type)
{
        std::string query =
                "INSERT INTO \"circuit\" (id, parent_scene, location_x, location_y, parent_circuit, gate_type) VALUES ("
                + std::string("'") + xto_string(id) + std::string("'") + ", " + std::string("'") +
                xto_string(parent_scene) + std::string("'") + ", " + xto_string(location_x) + ", " +
                xto_string(location_y) + ", " + (parent_circuit
                                                         ? (std::string("'") + xto_string(*parent_circuit) +
                                                            std::string("'"))
                                                         : "null") + ", " + std::string("'") + xto_string(gate_type) +
                std::string("'") + ")";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}

[[nodiscard]] inline bool circuit_update(Database &db, std::string /*PK*/ id, std::string parent_scene, int location_x,
                                         int location_y, std::optional<std::string> parent_circuit,
                                         std::string gate_type)
{
        std::string query = "UPDATE \"circuit\" SET parent_scene = " + std::string("'") + xto_string(parent_scene) +
                            std::string("'") + ", location_x = " + xto_string(location_x) + ", location_y = " +
                            xto_string(location_y) + ", parent_circuit = " + (
                                    parent_circuit
                                            ? (std::string("'") + xto_string(*parent_circuit) + std::string("'"))
                                            : "null") + ", gate_type = " + std::string("'") + xto_string(gate_type) +
                            std::string("'") + " WHERE id = " + std::string("'") + xto_string(id) + std::string("'") +
                            ";";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}
...
[[nodiscard]] inline bool circuit_save(Database &db, std::string /*PK*/ id, std::string parent_scene, int location_x,
                                       int location_y, std::optional<std::string> parent_circuit, std::string gate_type)
{
        circuit tmp;
...
[[nodiscard]] inline bool wire_insert(Database &db, std::string /*PK*/ id, std::string source_circuit,
                                      std::string target_circuit, bool init_signal, int amount_input, int amount_output,
                                      std::string location)
{
        std::string query =
                "INSERT INTO \"wire\" (id, source_circuit, target_circuit, init_signal, amount_input, amount_output, location) VALUES ("
                + std::string("'") + xto_string(id) + std::string("'") + ", " + std::string("'") +
                xto_string(source_circuit) + std::string("'") + ", " + std::string("'") + xto_string(target_circuit) +
                std::string("'") + ", " + xto_string(init_signal) + ", " + xto_string(amount_input) + ", " +
                xto_string(amount_output) + ", " + std::string("'") + xto_string(location) + std::string("'") + ")";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}

[[nodiscard]] inline bool wire_update(Database &db, std::string /*PK*/ id, std::string source_circuit,
                                      std::string target_circuit, bool init_signal, int amount_input, int amount_output,
                                      std::string location)
{
        std::string query = "UPDATE \"wire\" SET source_circuit = " + std::string("'") + xto_string(source_circuit) +
                            std::string("'") + ", target_circuit = " + std::string("'") + xto_string(target_circuit) +
                            std::string("'") + ", init_signal = " + xto_string(init_signal) + ", amount_input = " +
                            xto_string(amount_input) + ", amount_output = " + xto_string(amount_output) +
                            ", location = " + std::string("'") + xto_string(location) + std::string("'") +
                            " WHERE id = " + std::string("'") + xto_string(id) + std::string("'") + ";";
        return finalize_op(dao_query(db, query, PGRES_COMMAND_OK));
}
...
[[nodiscard]] inline bool wire_save(Database &db, std::string /*PK*/ id, std::string source_circuit,
                                    std::string target_circuit, bool init_signal, int amount_input, int amount_output,
                                    std::string location)
{
        wire tmp;
        if (!get_one_wire(db, &tmp, id))
                return wire_insert(db, id, source_circuit, target_circuit, init_signal, amount_input, amount_output,
                                   location);
...
{
        std::string query = "UPDATE scene SET scene_name = '" + xto_string(new_name) + "', description = '" +
                            xto_string(new_description) + "' WHERE id = '" + xto_string(id) + "' and author = '" +
                            xto_string(user_id) + "';";
...
{
        std::string query = "DELETE FROM scene s WHERE s.id = '" + xto_string(id) + "' AND s.author = '" +
                            xto_string(user_id) + "';";
        PGresult *res = dao_query(db, query, PGRES_COMMAND_OK);
        if (!res) return false;
        PQclear(res);
...
{
        std::string query = "update circuit set gate_type = '" + xto_string(gate_type) + "' where id = '" +
                            xto_string(id) + "';";
        PGresult *res = dao_query(db, query, PGRES_COMMAND_OK);
        if (!res) return false;
        PQclear(res);
...
{
        std::string query = "update circuit set location_y = " + xto_string(location_y) + " where id = '" +
                            xto_string(id) + "';";
        PGresult *res = dao_query(db, query, PGRES_COMMAND_OK);
        if (!res) return false;
        PQclear(res);
...
{
        std::string query = "update circuit set location_x = " + xto_string(location_x) + " where id = '" +
                            xto_string(id) + "';";
        PGresult *res = dao_query(db, query, PGRES_COMMAND_OK);
        if (!res) return false;
        PQclear(res);
...
{
        std::string query = "update circuit set location_x = " + xto_string(location_x) + ", location_y = " +
                            xto_string(location_y) + " where id = '" + xto_string(id) + "';";
        PGresult *res = dao_query(db, query, PGRES_COMMAND_OK);
        if (!res) return false;
        PQclear(res);
        return true;
...
{
        std::string query =
                "SELECT * FROM \"user\" u INNER JOIN session s on s.user_id = u.id WHERE s.session_token = '" +
                xto_string(user_id) + "';";
        PGresult *res = dao_query(db, query, PGRES_TUPLES_OK);
        if (!res) return false;
        if (PQntuples(res) != 1) {
...
{
        std::string query = "UPDATE session s SET s.invalidated = true WHERE s.session_token = '" +
                            xto_string(session_token) + "';";
        PGresult *res = dao_query(db, query, PGRES_COMMAND_OK);
        if (!res) return false;
        PQclear(res);
        return true;
