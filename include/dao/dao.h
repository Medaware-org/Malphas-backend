...
[[nodiscard]] inline bool wire_update_location(Database &db, std::string location, std::string id)
{
	std::string query = "update wire set location = '" + xto_string(location) + "' where id = '" + xto_string(id) + "';";
...
[[nodiscard]] inline bool wire_update_num_output(Database &db, int amount_output, std::string id)
{
	std::string query = "update wire set amount_output = " + xto_string(amount_output) + " where id = '" + xto_string(id) + "';";
	PGresult *res = dao_query(db, query, PGRES_COMMAND_OK);
	if (!res) return false;
	PQclear(res);
	return true;
...
[[nodiscard]] inline bool get_wires_in_scene(Database &db, std::vector<wire> &dst, std::string scene, std::string scene2)
{
	std::string query = "SELECT w.* FROM wire w INNER JOIN circuit c1 on c1.id = w.source_circuit INNER JOIN circuit c2 on c2.id = w.target_circuit WHERE c1.parent_scene = '" + xto_string(scene) + "' AND c2.parent_scene = '" + xto_string(scene2) + "';";
	PGresult *res = dao_query(db, query, PGRES_TUPLES_OK);
	if (!res) return false;
	dao_map_all<wire>(res, dst, [](auto *res, auto tuple) { return dao_map_wire(res, tuple); });
	PQclear(res);
...
[[nodiscard]] inline bool wire_update_num_input(Database &db, int amount_input, std::string id)
{
	std::string query = "update wire set amount_input = " + xto_string(amount_input) + " where id = '" + xto_string(id) + "';";
	PGresult *res = dao_query(db, query, PGRES_COMMAND_OK);
	if (!res) return false;
	PQclear(res);
	return true;
...
[[nodiscard]] inline bool wire_update_init_signal(Database &db, bool init_signal, std::string id)
{
	std::string query = "update wire set init_signal = " + xto_string(init_signal) + " where id = '" + xto_string(id) + "';";
	PGresult *res = dao_query(db, query, PGRES_COMMAND_OK);
	if (!res) return false;
	PQclear(res);
	return true;
